...
struct AckEvidence {
    // Single ACK format (modern): produced by `share_verifier`, includes share
    // commitments/digests, and is signed over a long binding message.
...
int main(int argc, char* argv[]) {
...

    {
        ...
        for (const auto& entry : fs::directory_iterator(acks_dir)) {
            ...
            if (ack.protocol_version != protocol_version || ack.schema_id != schema_id) continue;
